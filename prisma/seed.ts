...
const config = {
  ...
  // For development/testing - use custom password if provided
  customPassword: process.env.ROOT_ADMIN_PASSWORD || "admin@1234",
...
async function createRootAdmin(): Promise<{ user: any; password: string }> {
  return await prisma.$transaction(async (tx) => {
    ...
    // Use custom password for development or generate secure password
    let password: string;

    if (config.customPassword && !config.isProduction) {
      password = config.customPassword;
      console.log("🔐 Using custom password for development/testing");
      console.log(
        "⚠️  WARNING: Custom password may not meet security requirements"
      );

      // Validate custom password and show warnings
      const validation = validatePassword(password);
      if (!validation.valid) {
        console.log("❌ Password validation issues:");
        validation.errors.forEach((error) => console.log(`   • ${error}`));
        console.log("⚠️  Consider using a stronger password for production");
      }
    } else {
      // Generate secure password for production or when no custom password
      let attempts = 0;
